...
namespace orc {
  ...
  std::unique_ptr<Type> convertType(const facebook::velox::dwrf::proto::orc::Type& type,
                                    const facebook::velox::dwrf::proto::orc::Footer& footer) {
    ...
    case facebook::velox::dwrf::proto::orc::Type_Kind_UNION: {
      ...
      std::unique_ptr<Type> return_value = std::unique_ptr<Type>(result);
      if (type.kind() == facebook::velox::dwrf::proto::orc::Type_Kind_LIST && type.subtypes_size() != 1)
        throw ParseError("Illegal LIST type that doesn't contain one subtype");
      if (type.kind() == facebook::velox::dwrf::proto::orc::Type_Kind_MAP && type.subtypes_size() != 2)
        throw ParseError("Illegal MAP type that doesn't contain two subtypes");
      if (type.kind() == facebook::velox::dwrf::proto::orc::Type_Kind_UNION && type.subtypes_size() == 0)
