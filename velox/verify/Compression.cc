...
#ifdef VELOX_ENABLE_ISAL
  inflate_state zstream;
#else
    z_stream zstream;
#endif
    std::unique_ptr<DataBuffer<char>> buffer{nullptr};
  ...
  bool ZlibDecompressionStream::Next(const void** data, int*size) {
    ...
    if (state == DECOMPRESS_ORIGINAL) {
      ...
      *size = static_cast<int>(availSize);
      ...
      if (!buffer || blockSize > buffer->capacity()) {
        buffer = std::make_unique<DataBuffer<char>>(pool, blockSize);
      }
#ifdef VELOX_ENABLE_ISAL
      isal_inflate_reset(&zstream);
#else
      if (inflateReset(&zstream) != Z_OK) {
        throw std::logic_error("Bad inflateReset in ZlibDecompressionStream::Next");
      }
#endif
      zstream.next_in =
...
#ifdef VELOX_ENABLE_ISAL
      isal_inflate_stateless(&zstream);
      // TODO chenxu14
#else
      int64_t result;
      do {
        result = inflate(
            &zstream, availSize == remainingLength ? Z_FINISH : Z_SYNC_FLUSH);
        switch (result) {
        case Z_OK:
          ...
          readBuffer(true);
          availSize = std::min(
              static_cast<size_t>(inputBufferEnd - inputBuffer),
              remainingLength);
          zstream.next_in =
              reinterpret_cast<Bytef*>(const_cast<char*>(inputBuffer));
          zstream.avail_in = static_cast<uInt>(availSize);
