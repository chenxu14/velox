...
namespace orc {
  ...
  StripeStreamsImpl::StripeStreamsImpl(const RowReaderImpl& _reader, uint64_t _index,
                                       const facebook::velox::dwrf::proto::orc::StripeInformation& _stripeInfo,
                                       const facebook::velox::dwrf::proto::orc::StripeFooter& _footer,
  ...
  facebook::velox::dwrf::proto::orc::ColumnEncoding StripeStreamsImpl::getEncoding(uint64_t columnId
                                                       ) const {
  ...
  StripeStreamsImpl::getStream(uint64_t columnId,
                               facebook::velox::dwrf::proto::orc::Stream_Kind kind,
    ...
    for(int i = 0; i < footer.streams_size(); ++i) {
      const facebook::velox::dwrf::proto::orc::Stream& stream = footer.streams(i);
      if (stream.has_kind() &&
          stream.kind() == kind &&
    ...
    if (stripeFooter.get() == nullptr) {
      std::unique_ptr<SeekableInputStream> pbStream =
        createDecompressor(compression,
                           ...
                           blockSize,
                           memory);
      stripeFooter.reset(new facebook::velox::dwrf::proto::orc::StripeFooter());
