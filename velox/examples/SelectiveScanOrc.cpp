...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < options.iter; i++) {
    ...
    auto& type = reader->typeWithId()->type;
    const std::shared_ptr<const RowType>& rowType =
        std::dynamic_pointer_cast<const RowType>(type);
    VectorPtr batch = BaseVector::create(type, 0, pool.get());

    auto scanSpec = std::make_shared<common::ScanSpec>("root");
    // Which Column to Select
    uint64_t columnIndex = 4;
    auto child = scanSpec->addFieldRecursively(
        rowType->nameOf(columnIndex), *type->childAt(columnIndex), columnIndex);
    child->setExtractValues(true);
    child->setFilter(
        std::make_unique<common::BigintRange>(20000000000000, 50000000000000, false));
    rowReaderOptions.setScanSpec(scanSpec);
    std::vector<uint64_t> nodes = {columnIndex + 1};
    rowReaderOptions.select(std::make_shared<ColumnSelector>(rowType, nodes, true));
    ...
    while (rowReader->next(options.batch, batch)) {
      ...
      rows += batch->size();
      if (options.verbose) {
        auto colBatch = std::dynamic_pointer_cast<FlatVector<int64_t>>(
            BaseVector::loadedVectorShared(
                std::dynamic_pointer_cast<RowVector>(batch)->childAt(columnIndex)));
        for (size_t i = 0; i < batch->size(); ++i) {
          std::cout << colBatch->valueAt(i) << std::endl;
        }
      }
    }
